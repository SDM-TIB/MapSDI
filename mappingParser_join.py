...
def handler():
	...
	path = sys.argv[1] 
	output = sys.argv[2]
	source_attribute1 = dict()
	source_column1 = dict()
	source_attribute2 = dict()
	source_column2 = dict()
	concept_list1 = []
	concept_list2 = []
	dup_list1 = []
	...
	for file in os.listdir(path):
		...
		for item in mapping_list:
			if item.triples_map_id.split("/")[3] == "TripleMap1":
				...
				if concept in concept_list1:
					dup_list1.append(concept)
				else:
					concept_list1.append(concept)
				source_attribute1[source] = concept
				source_column1[source] = column
	
	for k,v in source_attribute1.items():
		if v in dup_list1:
			header1=v
			column_result1[k]=source_column1[k]
	...
	for file in os.listdir(path):
		mapping_list=mapping_parser(path+file)
		for item in mapping_list:
			if item.triples_map_id.split("/")[3] == "TripleMap2":
				source = item.data_source
				#print (source)
				sub_class = item.subject_map.rdf_class
				#print (sub_class)
				...
				l = len(str(sub_class).split("/"))
				concept = str(sub_class).split("/")[l-1]
				if concept in concept_list2:
					dup_list2.append(concept)
				else:
					concept_list2.append(concept)
				source_attribute2[source] = concept
				source_column2[source] = column

	for k,v in source_attribute2.items():
		if v in dup_list2:
			header2= v
			column_result2[k]=source_column2[k]		
